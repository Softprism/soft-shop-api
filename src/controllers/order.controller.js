...
const createOrder = async (req, res, next) => {
  ...
  if (newOrder.err) return res.status(500).json({ success: false, msg: newOrder.err });
...
const toggleFavorite = async (req, res, next) => {
  ...
  if (favoriteOrder.err) return res.status(500).json({ success: false, msg: favoriteOrder.err });
...
const getOrderDetails = async (req, res, next) => {
  ...
  if (orderDetails.err) return res.status(500).json({ success: false, msg: orderDetails.err });
...
const editOrder = async (req, res, next) => {
  ...
  if (updatedOrder.err) return res.status(500).json({ success: false, msg: updatedOrder.err });
